...
FROM country_club.Bookings b, country_club.Members m, country_club.Facilities f
...
select inner_q.facility_name, inner_q.member_name, inner_q.cost 
	from
		(SELECT f.name as facility_name, 
				concat(m.firstname,' ',m.surname) as member_name, 
				case when m.memid != 0 then b.slots * f.membercost
	...
	where inner_q.cost > 30
	order by inner_q.cost desc

/* Q10: Produce a list of facilities with a total revenue less than 1000.
The output of facility name and total revenue, sorted by revenue. Remember
that there's a different cost for guests and members! */
select inner_q.name as facility_name,
		sum(inner_q.cost) as total_revenue 
	from 
	(
		SELECT f.name,
				case when m.memid != 0 then b.slots * f.membercost
